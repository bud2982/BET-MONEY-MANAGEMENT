...
// Simple authentication middleware that simulates a premium user
const authMiddleware = (req: any, res: any, next: any) => {
  // Simulate authenticated premium user for demonstration
  req.user = {
    id: 'premium-user-123',
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get('/api/devices', authMiddleware, async (req: any, res) => {
  ...
  app.get("/api/sessions", authMiddleware, premiumMiddleware, async (req, res) => {
    try {
  ...
  app.post("/api/sessions", authMiddleware, premiumMiddleware, async (req, res) => {
    try {
  ...
  app.patch("/api/sessions/:id", authMiddleware, premiumMiddleware, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      if (isNaN(id)) {
        return res.status(400).json({ message: "Invalid session ID" });
  ...
  app.delete("/api/sessions/:id", authMiddleware, premiumMiddleware, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      if (isNaN(id)) {
        return res.status(400).json({ message: "Invalid session ID" });
  ...
  app.get("/api/sessions/:sessionId/bets", authMiddleware, premiumMiddleware, async (req, res) => {
    try {
  ...
  app.post("/api/sessions/:sessionId/bets", authMiddleware, premiumMiddleware, async (req, res) => {
    try {
      const sessionId = parseInt(req.params.sessionId);
      if (isNaN(sessionId)) {
        return res.status(400).json({ message: "Invalid session ID" });
  ...
  app.post('/api/create-demo-invite', authMiddleware, async (req, res) => {
    try {
  ...
  app.get('/api/demo-invites', authMiddleware, async (req, res) => {
    try {
